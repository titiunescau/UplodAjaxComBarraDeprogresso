...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:luci\n' // Nama kamu
            + 'ORG:Lexa;\n' // Nama bot
            + 'TEL;type=CELL;type=VOICE;waid=+5514996292192:+55 14996292192\n' //Nomor whatsapp kamu
            + 'END:VCARD'
prefix = '.','!','#','*','/'
blocked = []
limitawal = '999999999'
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Olá @${num.split('@')[0]}\nBem vindo ao grupo *${mdata.subject}*\n\nEspero que goste e se adapte ao grupo e não se esqueça de ler as regrinhas ❤️`
				...
				teks = `Ala, o trouxa do @${num.split('@')[0]} foi expulso do grupo *${mdata.subject}*\n!\n\n_Quem mandou fazer merda no grupo, agora nunca mais volte 😂👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
		...
		try {
			...
			mess = {
				...
				only: {
					...
					mod: '[❗] ESTE PEDIDO É ESPECÍFICO PARA USUARIO MOD Reinhard BOT*',
			...
			const ownerNumber = ["5514996292192@s.whatsapp.net"] // replace this with your number
			const mod = [ownerNumber,"5514996292192@s.whatsapp.net"]//ubah nomor lo
			const adminbotnumber = ["5514996292192@s.whatsapp.net"]//ubah nomor lo
			const frendsowner = ["5514996292192@s.whatsapp.net"]//ubah nomor lo
			const premium = ["5514996292192@s.whatsapp.net","5514996292192@s.whatsapp.net","559887053394@s.whatsapp.net"] 
		...
		if (messagesC.includes("bot")){
			...
			switch(command) {
	            ...
	            case 'help':
                    ...
                    var setP = [`https://i.ibb.co/HXcxKth/a-garota-Uzumaki.gif`,`https://i.ibb.co/gzhxmPb/STK-20210604-WA0113.webp`]
                const getFoto = setP[Math.floor(Math.random() * setP.length)]
                const luci = await getBuffer(getFoto)
                client.sendMessage(from, luci, image, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "caption": "⚡Reinhard bot⚡", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('./sticker/kkk.webp')} } }, caption: help(prefix, sender, pushname, time)})
                  break 
				case 'help1':
				case 'menu1':
...
teks = body.slice(13)
nescu = await getBuffer(`https://hadi-api.herokuapp.com/api/photoxy/manga-naruto?teks=${teks}`)
client.sendMessage(from, nescu, image, {quoted: mek})
break

case 'attp1':	
if (args.length < 1) return reply(`_Coloque o texto _\n\n*Exemplo ${prefix}sttc reinhard*`)
teks = body.slice(6)
...
break
				...
				case 'wibu':
                   ...
                   break
					case 'totaluser':
					if (!isOwner) return reply(mess.only.ownerB)    
					teks = `\`\`\`╭────*「 *TOTAL DE USUÁRIOS ReinhardBOT 👑* 」\n\`\`\``
					no = 0
          ...
          reply('Espere..')
           ...
           case 'shitlist':
               ...
               break
				...
				case 'dono':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://i.ibb.co/HXcxKth/a-garota-Uzumaki.gif`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*CRIADOR:* Luci\n*WPP:* wa.me/+55149962921\n*FACE:* https://www.facebook.com/Adolescentessoladospelavida/\n\n\nEspero que tenham gostado do bot 🐊🚩️'})
					break
				case 'setprefix':
					if (args.length < 1) return
					if (!isOwner) return reply(mess.only.ownerB)
